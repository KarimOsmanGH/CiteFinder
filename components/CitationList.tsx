...
interface CitationCardProps {
  citation: Citation
  index: number
  showGeneratedCitations?: boolean
}

function CitationCard({ citation, index, showGeneratedCitations = false }: CitationCardProps) {
  ...
  return (
    ...
    >
      ...
      {/* Generated Citations */}
      {showGeneratedCitations && citation.authors && citation.year && citation.title && (
        <div className="mt-4 p-4 bg-green-50 rounded-lg border border-green-200">
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              ...
              <button 
                onClick={() => handleCopy(`${citation.authors} (${citation.year}). ${citation.title}.`)}
            ...
            <div className="flex items-center justify-between">
              <div className="text-sm">
                ...
                <span className="text-green-700 ml-2">{citation.authors}. &ldquo;{citation.title}.&rdquo; ({citation.year}).</span>
              </div>
              <button 
                onClick={() => handleCopy(`${citation.authors}. "${citation.title}." (${citation.year}).`)}
                className="px-2 py-1 bg-green-600 text-white text-xs rounded hover:bg-green-700 transition-colors"
              >
                Copy
              </button>
